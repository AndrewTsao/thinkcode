...
var JSLINT = ( function() {
	var adsafe_id, adsafe_may, adsafe_went, anonname, approved, atrule = {
		...
		'/' : '\\/',
		'\\' : '\\\\'
	}, funct, functionicity = [ 'closure', 'exception', 'global', 'label',
			'outer', 'unused', 'var' ], functions, global, htmltag = {
		a : {},
		abbr : {},
		acronym : {},
		address : {},
	...
	}, strict_mode, syntax = {}, tab, token, urls, warnings, widget = {
		alert : true,
		animator : true,
		appleScript : true,
		beep : true,
	...
	String.prototype.name = function() {
		...
		if (nx.test(this)) {
			return '"' + this.replace(nxg,
					function(a) {
						var c = escapes[a];
						if (c) {
							return c;
						}
						return '\\u' + ('0000' + a.charCodeAt().toString(16))
								.slice(-4);
					}) + '"';
		}
		return '"' + this + '"';
	...
	var lex = ( function lex() {
		...
		function it(type, value) {
			...
			if (type === '(identifier)') {
				...
				if (value === '__iterator__' || value === '__proto__') {
					errorAt("Reserved name '{a}'.", line, from, value);
						...
						&& (value.charAt(0) === '_' || value
								.charAt(value.length - 1) === '_')) {
			...
			if (i !== '(endline)') {
				prereg = i
						&& (('(,=:[!&|?{};'.indexOf(i.charAt(i.length - 1)) >= 0) || i === 'return');
			}
			return t;
		}
		return {
			...
			token : function() {
				...
				function string(x) {
					var c, j, r = '';
					if (jsonmode && x !== '"') {
						warningAt("Strings must use doublequote.", line,
								character);
					}
					...
					function esc(n) {
						...
						if (i >= 32 && i <= 126 && i !== 34 && i !== 92
								&& i !== 39) {
							warningAt("Unnecessary escapement.", line,
									character);
						}
						character += n;
					...
					for (;;) {
						...
						if (c < ' ') {
							...
							if (option.safe && xmode === 'html') {
								warningAt("ADsafe string violation.", line,
										character + j);
							...
							if (xmode === 'html') {
								...
								case 'x':
									if (jsonmode) {
										warningAt("Avoid \\x-.", line,
												character);
									}
									esc(2);
									break;
								default:
									warningAt("Bad escapement.", line,
											character);
								}
							}
						}
				...
				for (;;) {
					...
					if (!t) {
						if (xmode === 'html') {
							...
							if (s) {
								errorAt("Unexpected '{a}'.", line, character, s
										.substr(0, 1));
							}
						}
					} else {
						...
						if (c.isDigit()) {
							if (xmode !== 'style' && !isFinite(Number(t))) {
								warningAt("Bad number '{a}'.", line, character,
										t);
							}
							if (xmode !== 'style' && xmode !== 'styleproperty'
									&& s.substr(0, 1).isAlpha()) {
								warningAt("Missing space after '{a}'.", line,
										character, t);
							}
							if (c === '0') {
								d = t.substr(1, 1);
						...
						case '//':
							if (src || (xmode && xmode !== 'script')) {
								warningAt("Unexpected comment.", line,
										character);
							} else if (xmode === 'script' && /<\s*\//i.test(s)) {
								warningAt("Unexpected <\/ in comment.", line,
										character);
							} else if ((option.safe || xmode === 'script')
									&& ax.test(s)) {
						...
						case '<!--':
							...
							for (;;) {
								...
								i = s.indexOf('<!');
								if (i >= 0) {
									errorAt("Nested HTML comment.", line,
											character + i);
								}
								if (!nextLine()) {
							...
							l = s.indexOf('<!');
							if (l >= 0 && l < i) {
								errorAt("Nested HTML comment.", line, character
										+ l);
							}
							character += i;
							if (s[i + 2] !== '>') {
		...
		if (option.adsafe) {
			...
			case 'lib':
				if (nexttoken.value === 'ADSAFE') {
					...
					f = parse(0);
					if (f.id !== 'function') {
						error('The second argument to lib must be a function.',
								f);
					}
					p = f.funct['(params)'];
					p = p && p.join(', ');
					if (p && p !== 'lib') {
						error("Expected '{a}' and instead saw '{b}'.", f,
								'(lib)', '(' + p + ')');
					}
					advance(')');
					advance(';');
				...
				for (;;) {
					if (nexttoken.identifier) {
						...
						case 'url':
							...
							break;
						case 'expression':
							warning("Unexpected expression '{a}'.", nexttoken,
									nexttoken.value);
							advance();
							break;
						default:
	...
	cssBorderStyle = [ 'none', 'hidden', 'dotted', 'dashed', 'solid', 'double',
			'ridge', 'inset', 'outset' ];
	cssBreak = [ 'auto', 'always', 'avoid', 'left', 'right' ];
	cssOverflow = [ 'auto', 'hidden', 'scroll', 'visible' ];
	cssAttributeData = {
		background : [ true, 'background-attachment', 'background-color',
				'background-image', 'background-position', 'background-repeat' ],
		'background-attachment' : [ 'scroll', 'fixed' ],
		'background-color' : [ 'transparent', cssColor ],
		'background-image' : [ 'none', cssUrl ],
		'background-position' : [ 2,
				[ cssLength, 'top', 'bottom', 'left', 'right', 'center' ] ],
		'background-repeat' : [ 'repeat', 'repeat-x', 'repeat-y', 'no-repeat' ],
		'border' : [ true, 'border-color', 'border-style', 'border-width' ],
		'border-bottom' : [ true, 'border-bottom-color', 'border-bottom-style',
				'border-bottom-width' ],
		...
		'border-left' : [ true, 'border-left-color', 'border-left-style',
				'border-left-width' ],
		'border-left-color' : cssColor,
		'border-left-style' : cssBorderStyle,
		'border-left-width' : cssWidth,
		'border-right' : [ true, 'border-right-color', 'border-right-style',
				'border-right-width' ],
		...
		'border-top' : [ true, 'border-top-color', 'border-top-style',
				'border-top-width' ],
		...
		content : [ 'open-quote', 'close-quote', 'no-open-quote',
				'no-close-quote', cssString, cssUrl, cssCounter, cssAttr ],
		'counter-increment' : [ cssName, 'none' ],
		'counter-reset' : [ cssName, 'none' ],
		cursor : [ cssUrl, 'auto', 'crosshair', 'default', 'e-resize', 'help',
				'move', 'n-resize', 'ne-resize', 'nw-resize', 'pointer',
				's-resize', 'se-resize', 'sw-resize', 'w-resize', 'text',
				'wait' ],
		direction : [ 'ltr', 'rtl' ],
		display : [ 'block', 'compact', 'inline', 'inline-block',
				'inline-table', 'list-item', 'marker', 'none', 'run-in',
				'table', 'table-caption', 'table-cell', 'table-column',
				'table-column-group', 'table-footer-group',
				'table-header-group', 'table-row', 'table-row-group' ],
		'empty-cells' : [ 'show', 'hide' ],
		'float' : [ 'left', 'none', 'right' ],
		font : [ 'caption', 'icon', 'menu', 'message-box', 'small-caption',
				'status-bar', true, 'font-size', 'font-style', 'font-weight',
				'font-family' ],
		'font-family' : cssCommaList,
		'font-size' : [ 'xx-small', 'x-small', 'small', 'medium', 'large',
				'x-large', 'xx-large', 'larger', 'smaller', cssLength ],
		...
		'list-style' : [ true, 'list-style-image', 'list-style-position',
				'list-style-type' ],
		...
		'list-style-type' : [ 'circle', 'disc', 'square', 'decimal',
				...
				'upper-latin', 'hebrew', 'katakana', 'hiragana-iroha',
				'katakana-oroha', 'none' ],
		margin : [ 4, cssMargin ],
		...
		'outline-style' : [ 'dashed', 'dotted', 'double', 'groove', 'inset',
		...
		'vertical-align' : [ 'baseline', 'bottom', 'sub', 'super', 'top',
				'text-top', 'middle', 'text-bottom', cssLength ],
	...
	function stylePattern() {
		var name;
		if (nexttoken.id === '{') {
			warning("Expected a style pattern, and instead saw '{a}'.",
					nexttoken, nexttoken.id);
	...
	function doAttribute(n, a, v) {
		...
		if (a === 'id') {
			...
			x = v.search(dx);
			if (x >= 0) {
				warning("Unexpected character '{a}' in {b}.", token, v
						.charAt(x), a);
			}
			ids[u] = true;
			...
			ids[u] = true;
		} else if (a === 'href' || a === 'background' || a === 'content'
				|| a === 'data' || a.indexOf('src') >= 0
				|| a.indexOf('url') >= 0) {
	...
	function doTag(n, a) {
		...
		if (stack.length > 0) {
			...
			if (x) {
				...
				do {
					if (i <= 0) {
						error("A '<{a}>' must be within '<{b}>'.", token, n,
								'body');
					}
					i -= 1;
	...
	function html() {
		...
		for (;;) {
			...
			case '<':
				...
				for (;;) {
					...
					if (!option.cap && a !== a.toLowerCase()) {
						warning("Attribute '{a}' not all lower case.",
								nexttoken, a);
					}
					a = a.toLowerCase();
					xquote = '';
					...
					if (a.slice(0, 2) === 'on') {
						...
						if (nexttoken.id === '=') {
							advance('=');
							v = nexttoken.value;
							if (!nexttoken.identifier && nexttoken.id !== '"'
									&& nexttoken.id !== '\''
			...
			if (stack
					&& stack.length === 0
					&& (option.adsafe || !option.fragment || nexttoken.id === '(end)')) {
				break;
			}
		}
		if (nexttoken.id !== '(end)') {
			...
			function() {
				...
				if (c && c.id !== 'function') {
					if (c.identifier) {
						...
						case 'Math':
						case 'JSON':
							warning("Do not use {a} as a constructor.", token,
									c.value);
							break;
						case 'Function':
							if (!option.evil) {
	...
	infix(
			...
			function(left, that) {
				...
				advance(')');
				if (option.immed && left.id === 'function'
						&& nexttoken.id !== ')') {
					warning(
			...
			function(left, that) {
				...
				if (e && e.type === '(string)') {
					if (option.safe && banned[e.value] === true) {
						...
						warning("ADsafe restricted subscript '{a}'.", that,
								e.value);
					}
					countMember(e.value);
				...
				if (peek(nexttoken.id === 'var' ? 1 : 0).id === 'in') {
					if (nexttoken.id === 'var') {
						...
						case 'var':
							break;
						default:
							warning("Bad for in variable '{a}'.", nexttoken,
									nexttoken.value);
						}
						advance();
					}
			...
			if (nexttoken.id !== '}') {
				for (;;) {
					...
					if (o[nexttoken.value] === true) {
						warning("Duplicate key '{a}'.", nexttoken,
								nexttoken.value);
	...
	itself.data = function() {
		var data = {
			functions : []
		}, fu, globals, implieds = [], f, i, j, members = [], n, unused = [], v;
	...
	itself.report = function(option) {
		...
		if (data.errors || data.implieds || data.unused) {
			...
			if (data.errors) {
				for (i = 0; i < data.errors.length; i += 1) {
					...
					if (c) {
						e = c.evidence || '';
						o.push('<p>Problem'
								+ (isFinite(c.line) ? ' at line ' + c.line
			...
			if (data.implieds) {
				...
				for (i = 0; i < data.implieds.length; i += 1) {
					s[i] = '<code>' + data.implieds[i].name
							+ '</code>&nbsp;<i>' + data.implieds[i].line
							+ '</i>';
				}
			...
			if (data.unused) {
				...
				for (i = 0; i < data.unused.length; i += 1) {
					s[i] = '<code><u>' + data.unused[i].name
							+ '</u></code>&nbsp;<i>' + data.unused[i].line
							+ '</i> <code>' + data.unused[i]['function']
							+ '</code>';
				}
		...
		if (!option) {
			...
			if (data.json && !err) {
				...
				o
						.push('<div><i>Global</i> ' + data.globals.sort().join(
								', ') + '</div>');
			} else {
...
"use strict";
var ADSAFE = ( function() {
	var adsafe_id, adsafe_lib, banned = {
		'arguments' : true,
		callee : true,
		caller : true,
		constructor : true,
		...
		valueOf : true,
		watch : true
	}, cache_style_object, cache_style_node, defaultView = document.defaultView, ephemeral, flipflop, has_focus, hunter, interceptors = [], makeableTagName = {
		a : true,
		abbr : true,
		acronym : true,
		address : true,
		...
		ul : true,
		'var' : true
	}, name, pecker, result, star, the_range, value;
	...
	function reject(object, name) {
		return typeof object !== 'object'
				|| banned[name]
				|| ((typeof name !== 'number' || name < 0) && (typeof name !== 'string'
	...
	function getStyleObject(node) {
		...
		cache_style_object = node.currentStyle
	...
	function parse_query(text, id) {
		var match, query = [], selector, qx = id ? /^\s*(?:([\*\/])|\[\s*([a-z][0-9a-z_\-]*)\s*(?:([!*~|$\^]?\=)\s*([0-9A-Za-z_\-*%&;.\/:!]+)\s*)?\]|#\s*([A-Z]+_[A-Z0-9]+)|:\s*([a-z]+)|([.&_>\+]?)\s*([a-z][0-9a-z\-]*))\s*/
	...
	function make_root(root, id) {
		...
		var allow_focus = true, dom, dom_event = function(e) {
			var key, target, that, the_event, the_target, the_actual_event = e
					|| event, type = the_actual_event.type;
			...
			the_event = {
				...
				bubble : function() {
					try {
						var parent = that.getParent(), b = parent.___nodes___[0];
						that = parent;
						the_event.that = that;
						if (b['___ on ___'] && b['___ on ___'][type]) {
							that.fire(the_event);
						} else {
							the_event.bubble();
						}
		...
		Bunch.prototype = {
			append : function(appendage) {
				...
				var b = this.___nodes___, flag = false, i, j, node, rep;
				...
				if (appendage instanceof Array) {
					...
					for (i = 0; i < b.length; i += 1) {
						node = b[i];
						for (j = 0; j < rep.length; j += 1) {
							node.appendChild(flag ? rep[j].cloneNode(true)
									: rep[j]);
						}
						flag = true;
					}
			...
			fire : function(event) {
				var array, b, i, j, n, node, on, type;
				if (this === this.window) {
					return error();
				}
			...
			selection : function(string) {
				...
				if (b.length === 1 && allow_focus) {
					...
					if (typeof node.selectionStart === 'number') {
						...
						old = node.value;
						node.value = old.slice(0, start) + string
								+ old.slice(end);
					...
					for (i = 0; i < b.length; i += 1) {
						...
						if (node.tagName) {
							if (name !== 'float') {
								...
								node.style.cssFloat = node.style.styleFloat = value[i];
							}
						}
					}
				} else {
					for (i = 0; i < b.length; i += 1) {
						...
						if (node.tagName) {
							if (name !== 'float') {
								...
								node.style.cssFloat = node.style.styleFloat = String(value);
							}
						}
					}
				}
		...
		dom = {
			...
			row : function(values) {
				var tr = document.createElement('tr'), td, i;
		...
		if (typeof root.addEventListener === 'function') {
			...
			root.onfocusin = root.onfocusout = root.onmouseover = root.onmouseout = root.onmouseup = root.onmousedown = root.onmousemove = root.onclick = root.ondblclick = root.onkeypress = dom_event;
		}
	...
	return {
		create : typeof Object.create === 'function' ? Object.create
				: function(o) {
					F.prototype = typeof o === 'object' && o ? o
							: Object.prototype;
					return new F();
				},
...
ADSAFE._intercept( function(id, dom, lib, bunch) {
	lib.cookie = {
		...
		set : function(value) {
			var d, j = JSON.stringify(value).replace(/[=]/g, '\\u003d')
					.replace(/[;]/g, '\\u003b');
			if (j.length < 2000) {
				d = new Date();
